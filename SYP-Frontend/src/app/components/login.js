...
export default class Login extends Component{
    ...
    async onSubmitLogin(e){
        ...
        await axios.post('http://localhost:4000/users/login', loginCredentials)
            .then(res => {
            if(res.status===200){
    ...
    render(){
        return  <Form onSubmit={this.onSubmitLogin}>
                    <div className="form-group text-left font-paragragh">
                        ...
                        <input type="text" className="form-control font-paragragh-small" placeholder="Enter Email Address or Username" value={this.state.login_email} id="login_email" onChange={this.onChange}/>
                    </div>
                    <div className="form-group text-left font-paragragh">
                        <label>Password:</label>
                        <input type="password" className="form-control font-paragragh-small" placeholder="Enter Password" value={this.state.login_password} id="login_password" onChange={this.onChange}/>
                    </div>
